...
options.icons = {
  ...
  srcimg : options.theme.components + '06-libraries/images/',
  dest   : options.theme.build + 'icons/'
};

options.fonts = {
  src : options.rootPath.theme + 'font/',
};

// Set the URL used to access the Drupal website under development. This will
// allow Browser Sync to serve the website and update CSS changes on the fly.
options.drupalURL = 'http://myportal.docker.localhost';
...
options.styleGuide = {
  ...
  'css': [
    ...
    'kss-assets/base/css/marquee.css',
  ...
  'title': 'Style Guide for My Portal Theme'
};
...
gulp.task('styles', function () {
  return gulp.src(sassFiles)
    ...
    .pipe(gulpif(!isProd, sourcemaps.write('.', {
      ...
      sourceRoot: options.theme.components
    })))
    .pipe($.rename({dirname: ''}))
...
// #################
//
// Copy the mime icons from the components folder to the assets folder
// ===================================================
...
// #################
//
// Static images for theme
//
// #################
//
// Copy the images from the components folder to the assets folder
// ===================================================

gulp.task('images', function () {
  return gulp.src(options.icons.srcimg + '*')
...
// #################
//
// Copy the fonts from the fonts folder to the assets folder
// ===================================================

gulp.task('fonts', function () {
  return gulp.src(options.fonts.src + '**/*')
    .pipe(gulp.dest(options.theme.build + 'font/'))
});
...
gulp.task('scripts', function () {
  return gulp.src(options.theme.components + '**/*.js')
    ...
    .pipe(gulpif(isProd, $.uglify(), $.flatten() ))
    .pipe($.rename({
      suffix: '.min'
    }))
...
gulp.task('watch', ['watch:css', 'watch:js', 'watch:icons', 'images'], function () {
...
//
// ######################
gulp.task('default', ['styles', 'sprite-icons', 'scripts', 'images', 'mime-image-icons', 'fonts']);
