 ...
 */
function myportal_theme_form_system_theme_settings_alter(&$form, FormStateInterface &$form_state, $form_id = NULL) {
  ...
  // If the default theme is either myportal_theme or socialsaas then extend
  ...
  if (array_key_exists('socialbase', \Drupal::service('theme.manager')->getActiveTheme()->getBaseThemes())) {
    if ($active_theme == $system_theme_settings) {
      ...
      $form['#submit'][] = 'myportal_theme_save_email_logo';
      ...
      if (\Drupal::service('module_handler')->moduleExists('social_font')) {
        ...
        foreach (Font::loadMultiple() as $font_entities) {
          $id = $font_entities->id();
          if ($id != NULL) {
            $fonts[$id] = $font_entities->{'name'}->value;
          }
        }
      }
      ...
      if (\Drupal::configFactory()->get('system.theme')->get('admin') === 'gin') {
        $form['#submit'][] = 'myportal_theme_update_gin_color_settings';
      }
    }
...
/**
 * Updates the gin color settings from myportal_theme it's config.
 *
 ...
 */
function myportal_theme_update_gin_color_settings(array $form, FormStateInterface $form_state) {
  // Grab the default myportal_theme colors, these are set if the color settings
  // aren't overridden yet.
  $default_colors = \Drupal::configFactory()->getEditable('myportal_theme.settings')->getRawData();
  ...
  $myportal_theme_colors = \Drupal::configFactory()->getEditable('color.theme.myportal_theme')->getRawData();
  ...
  // default My Portal Theme settings.
  $brand_primary = !empty($myportal_theme_colors) ? $myportal_theme_colors['palette']['brand-primary'] : $default_colors['color_primary'];
  $brand_secondary = !empty($myportal_theme_colors) ? $myportal_theme_colors['palette']['brand-secondary'] : $default_colors['color_secondary'];
 ...
 * @throws \Drupal\Core\Entity\EntityStorageException
 */
function myportal_theme_save_email_logo(array $form, FormStateInterface $form_state) {
