...
use SocialConnect\OpenIDConnect\AccessToken as ODICAccessToken;

/**
 * Dummy access token with empty data.
 */
class KeycloakAccessToken extends ODICAccessToken implements AccessTokenInterface{
  ...
  public function __construct(array $token)
  {
    ...
    if (isset($token['refresh_expires_in'])) {
      if (!is_numeric($token['refresh_expires_in'])) {
        throw new InvalidAccessToken('refresh_expires_in value must be an integer');
      }

      $this->refresh_expires = $token['refresh_expires_in'] != 0 ? time() + $token['refresh_expires_in'] : 0;
    }
